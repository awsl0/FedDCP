...
#gcr_Head
class gcr_head(nn.Module):
    def __init__(self, dims=[8] * 100, in_features=1600, num_classes=10, gamma=25.0):
        super().__init__()
        self.gamma = gamma
        self.dims = dims
        self.num_classes = num_classes
        self.in_channels = in_features
        assert len(dims) == num_classes  # 判断dims 的长度是否等于 num_classes。如果不等，则表示 dims 中的维度数量与类别数量不匹配。
        assert len(np.unique(dims)) == 1  # 判断每个类的维度是否相同，如果不等，表示每个类别的维度数量不同
        self.subdim = dims[0]
        self.fc = nn.Linear(in_features, sum(dims), bias=False)
        self.fc.weight.geometry = dims

    def init_weights(self):
        with torch.no_grad():
            w = self.fc.weight
            nn.init.normal_(w) #这一步使用正态分布初始化权重 w
            wt = w.reshape(-1, self.subdim,
                           self.in_channels).permute([0, 2, 1])
            wt = torch.linalg.qr(wt).Q #这一步使用 QR 分解将矩阵 wt 分解为 Q 和 R 两个矩阵。其中，Q 是正交矩阵
            w = wt.permute([0, 2, 1]).reshape(-1, self.in_channels) #这一步将矩阵 wt 转置回原来的形状。
            self.fc.weight.add_(w - self.fc.weight) #将结果赋值给 fc 层的权重。

    def forward(self, x):
        out = nn.functional.normalize(x, dim=-1)
        cls_score = self.gamma * self.fc(out)
