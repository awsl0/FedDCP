...
from flcore.servers.serverGCR import FedGCR
from flcore.servers.serveravgDBE import FedAvgDBE
from flcore.servers.serverDCP import FedDCP
...
def run(args):
    ...
    for i in range(args.prev, args.times):
        ...
        elif args.algorithm == "Fedgcr":
            args.head = copy.deepcopy(args.model.fc)
            args.model.fc = nn.Identity()
            args.model = BaseHeadSplit(args.model, args.head)
            server = FedGCR(args, i)
        elif args.algorithm == "FedDCP":
            args.head = copy.deepcopy(args.model.fc)
            args.model.fc = nn.Identity()
            args.model = BaseHeadSplit(args.model, args.head)
            server = FedDCP(args, i)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-algo', "--algorithm", type=str, default="Fedgcr")
    ...
    # FedAvgDBE
    parser.add_argument('-mo', "--momentum", type=float, default=1)
    ...
    # FedDCP
    parser.add_argument('-ap', "--alpha", type=float, default=0.2)
