...
class RSGD(Optimizer):
    ...
    def __setstate__(self, state):
        ...
        for group in self.param_groups:
            group.setdefault('nesterov', False) #nesterov 参数控制是否使用 Nesterov 加速
            group.setdefault('maximize', False) #maximize 参数控制是否最大化损失函数。
            group.setdefault('foreach', None) # foreach 参数用于在更新参数时进行额外的处理
...
def _single_tensor_sgd(params: List[Tensor], d_p_list: List[Tensor],
    ...
    for i, param in enumerate(params):
        ...
        if hasattr(param, 'geometry'):
            if len(np.unique(param.geometry)) == 1: #可能是用来确定参数是否具有一致的几何结构
                ...
                # euclidean grad to riemannian grad 欧式梯度转换为黎曼梯度
                ...
                if version.parse(torch.__version__) < version.parse(
                    ...
                    (batch_U, batch_s, batch_Vt) = torch.linalg.svd(
                        batch_g.cpu(), full_matrices=False)# 对得到的梯度执行奇异值分解（SVD）
                ...
                new_batch_p = (
                    ...
                    batch_U * torch.sin(
                        (alpha * LR_FACTOR) * batch_s).reshape(-1, 1, subdim)
                ) @ batch_Vt    # 利用分解得到的结果计算新的参数new_batch_p,它是通过一系列的矩阵运算得到的，其中包括矩阵乘法，张量重塑，以及 torch.cos 和 torch.sin 函数。
